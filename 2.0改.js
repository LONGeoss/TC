...
await 请求日志记录(env, request, 访问IP, 'Admin_Login', config_JSON);
                ...
                // 【新增修改 C：强制覆盖 token 变量 (最强力方案)】
                // 1. **计算/获取正确的订阅 TOKEN**
                let 订阅TOKEN = await MD5MD5(url.host + userID);
                if (env.SUB_TOKEN) { // 只要设置了 SUB_TOKEN 就使用
                    订阅TOKEN = env.SUB_TOKEN; 
                }
                ...
                return fetch(Pages静态页面 + '/admin')
                    .then(async response => {
                        ...
                        // 强制覆盖 token 变量的 JavaScript 代码
                        const overrideScript = `
                            <script>
                                // 强制覆盖前端计算出的动态 token，确保使用 Worker 环境变量的值
                                window.token = "${订阅TOKEN}";
                                console.log('Token Override Successful:', window.token);
                            </script>
                        `;

                        // 找到 </body> 标签之前的位置进行插入，确保在所有 JS 执行完毕后执行
                        html = html.replace('</body>', overrideScript + '\n</body>');
                        
                        // (保留删除登录页版权的逻辑)
                        const textToReplace = /<div\s+class="footer-hint">[\s\S]*?<\/div>/i;
                        html = html.replace(textToReplace, '');
        ...
        // 【修改 B：固定订阅 TOKEN 逻辑 (无长度限制)】
        let 订阅TOKEN;
        if (env.SUB_TOKEN) { // 只要设置了 SUB_TOKEN 就使用
            订阅TOKEN = env.SUB_TOKEN;
        } else {
            // 否则，使用原有的 MD5 动态生成逻辑
            订阅TOKEN = await MD5MD5(url.host + userID);
        }
...
async function 读取config_JSON(env, host, userID, 重置配置 = false) {
    ...
    config_JSON.优选订阅生成.TOKEN = env.SUB_TOKEN;
